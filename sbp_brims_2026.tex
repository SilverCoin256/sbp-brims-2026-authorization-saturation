...
% Compress float spacing to meet the strict 10-page limit
...
% Compress bibliography spacing to meet the strict 10-page limit
...
While our queueing model can be represented as a formal special case of Delasay et al.~\cite{delasay2016}, who model load-dependent service rates, the conceptual and analytical focus diverges. Delasay et al. treat state dependency as an operational optimization problem for adaptive servers. In contrast, we analyze a \emph{sociotechnical phase transition} (decoupling) specifically driven by the authorization-production asymmetry of AI tools. Exogenous thresholds in prior models~\cite{delasay2016,kcterwiesch2009} are not parameterized as organizational tolerance $k$, nor do they derive a closed-form tipping point $\rho^* = k/(1+k)$ or map parameters to public administrative data. Our contribution is this sociotechnical bifurcation framework and its empirical calibration. Queueing
...
This threshold-switching models the classic organizational phenomenon of \emph{decoupling}~\cite{meyer1977}: once cognitive backlog exceeds the institutional tolerance threshold $T = kc$, human reviewers undergo a qualitative transition from slow, deliberate substantive reasoning (System 2) to rapid, checklist-based ceremonial heuristic processing (System 1) to maintain system throughput---a satisficing response to severe queue pressure~\cite{simon1947}. The parameter $k$ encodes this institutional tolerance before heuristic processing replaces deliberation; we interpret $k$ as a structural governance parameter measurable from override rates and review-time distributions, distinguishing it from the purely descriptive exogenous thresholds in prior behavioral queueing models~\cite{delasay2016,kcterwiesch2009}.
...
While the diffusion approximation holds asymptotically as $c \to \infty$ (fully capturing large-scale systems like the VBA where $c \approx 5{,}000$), our finite-capacity simulations demonstrate that even at small scale ($c = 5$), the tipping point remains structurally robust. This indicates that the closed-form threshold $\rho^*$ is an exceptionally robust first-order approximation for small administrative teams, with the stochastic variance merely broadening the transition band.
...
levels $k$ ($c=5$, $\alpha=3$, $q_s=1.0$, $q_c=0.25$). Curves are simulation means
(50 runs per point, 95\% CI as shaded bands). Dashed verticals mark $\rho^* \approx k/(1+k)$~\cite{halfin1981};
quality decline steepens at each threshold.}
\label{fig:quality}
\vspace{-10pt}
\end{figure}
...
\noindent\textit{Finite-System Convergence and Noise.} Rather than serving as a self-referential validation of the mathematical model, the simulation explores how finite-system stochastic fluctuations ($c = 5$) deviate from the asymptotic Halfin-Whitt limit. As expected, finite-server counts broaden the transition band around $\rho^*$ due to queueing variance, but the inflection remains centered on the analytical prediction $\rho^* = 0.50$, verifying that the asymptotic threshold behaves as a reliable structural attractor. Empirical calibration and counterfactual analysis follow below.
...
\subsection{Empirical Calibration and Policy Counterfactuals}
\label{sec:empirical}

To demonstrate the model's practical utility, we show how policy analysts can parameterize the system using standard, public administrative data to calibrate the model and identify saturation danger zones before deployment. Table~\ref{tab:operationalization} applies this calibration framework to the U.S. Veterans Benefits Administration (VBA)~\cite{va2024}.

\begin{table}[t]
\centering
\caption{Variable-to-observable mapping and calibration for the VBA. The baseline substantive rate ($\mu_s \approx 375$) is derived from historical OIG performance audits (approx.\ 1.5 claims completed per adjudicator per working day, multiplied by 250 working days/year). An estimated $\rho_s \approx 1.3 > 1.0$ indicates chronic operational overload.}
\label{tab:operationalization}
\begin{tabular}{@{}lll@{}}
\toprule
Variable & Observable Indicator & VBA estimate \\
\midrule
...
$k$. Rather than asserting a dynamic fit, this calibration acts as a generative counterfactual to diagnose the structural source of agency backlogs. The observable signatures are highly consistent with the model: a 125-day backlog threshold (operational $T$) and elevated appeal reversal rates (observable $q_c$). Under these overload conditions, the agency has historically resorted to mandatory overtime (I1)---a capacity intervention that temporarily reduces $\rho_s$ without shifting the structural threshold $\rho^*$, consistent with our model's prediction that capacity adjustments delay but cannot prevent saturation once workflow norms remain unchanged.
